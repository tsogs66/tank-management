...
const ImageCutout = (() => {
  ...
  /**
   * Scale an uploaded file down and keep it as PNG, so transparency survives.
   * Flattened to JPEG a signature would print as a white box over the sheet.
   */
  async function toPngDataUrl(file, maxEdge = 900) {
    ...
    try { return canvas.toDataURL('image/png'); } catch { return raw; }
  }
  ...
  async function removeBackground(srcDataUrl, {
    lo = 10, hi = 40, neutralLo = 26, neutralHi = 62, chroma = 1.3, pad = 6,
  } = {}) {
